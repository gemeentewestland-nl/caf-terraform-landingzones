...
module "container_app_environments" {
  ...
  log_analytics_workspace_id          = can(each.value.log_analytics_workspace.id) ? try(local.remote.diagnostics.log_analytics[each.value.diagnostic_log_analytics_workspace.key].id, each.value.log_analytics_workspace.id) : local.remote.diagnostics.log_analytics[try(each.value.log_analytics_workspace.lz_key, var.landingzone.key)][each.value.log_analytics_workspace.key].id
  log_analytics_primary_shared_key    = can(each.value.log_analytics_workspace.primary_shared_key) ? try(local.remote.diagnostics.log_analytics[each.value.diagnostic_log_analytics_workspace.key].id, each.value.log_analytics_workspace.id) : local.remote.diagnostics.log_analytics[try(each.value.log_analytics_workspace.lz_key, var.landingzone.key)][each.value.log_analytics_workspace.key].primary_shared_key

module "container_app_jobs" {
  ...
  container_app_environment_id        = can(each.value.container_app_environment_id) ? each.value.container_app_environment_id : module.container_app_environments[try(each.value.container_app_environment.lz_key, var.landingzone.key)][try(each.value.container_app_environment.key, each.value.container_app_environment_key)].id
